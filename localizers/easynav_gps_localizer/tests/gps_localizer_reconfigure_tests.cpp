...
class GpsLocalizerReconfigureTest : public ::testing::Test
{
protected:
  void SetUp() override
  {
...
TEST_F(GpsLocalizerReconfigureTest, InitializeTwiceOnSameNodeDoesNotThrow)
{
  auto node = rclcpp_lifecycle::LifecycleNode::make_shared("gps_localizer_reconfigure_test");

  auto plugin1 = std::make_shared<easynav::GpsLocalizer>();
  ASSERT_NO_THROW(plugin1->initialize(node, "test_localizer"));

  auto plugin2 = std::make_shared<easynav::GpsLocalizer>();
