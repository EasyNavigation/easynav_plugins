...
class FusionLocalizerReconfigureTest : public ::testing::Test
{
protected:
  void SetUp() override
  {
...
TEST_F(FusionLocalizerReconfigureTest, InitializeTwiceOnSameNodeDoesNotThrow)
{
  ...
  // FusionLocalizer::on_initialize() requires a LocalizerNode, not a plain LifecycleNode.
  auto node = std::make_shared<easynav::LocalizerNode>(options);

  auto plugin1 = std::make_shared<easynav::FusionLocalizer>();
  ASSERT_NO_THROW(plugin1->initialize(node, "test_localizer"));

  auto plugin2 = std::make_shared<easynav::FusionLocalizer>();
