...
class NavMapMapsManagerReconfigureTest : public ::testing::Test
{
protected:
  void SetUp() override
  {
...
TEST_F(NavMapMapsManagerReconfigureTest, InitializeTwiceOnSameNodeDoesNotThrow)
{
  auto node = rclcpp_lifecycle::LifecycleNode::make_shared(
    "navmap_maps_manager_reconfigure_test");

  auto plugin1 = std::make_shared<easynav::navmap::NavMapMapsManager>();
  ASSERT_NO_THROW(plugin1->initialize(node, "test_maps_manager"));

  auto plugin2 = std::make_shared<easynav::navmap::NavMapMapsManager>();
