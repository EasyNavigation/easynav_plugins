...
class RoutesMapsManagerReconfigureTest : public ::testing::Test
{
protected:
  void SetUp() override
  {
...
TEST_F(RoutesMapsManagerReconfigureTest, InitializeTwiceOnSameNodeDoesNotThrow)
{
  auto node = rclcpp_lifecycle::LifecycleNode::make_shared(
    "routes_maps_manager_reconfigure_test");

  auto plugin1 = std::make_shared<easynav::RoutesMapsManager>();
  ASSERT_NO_THROW(plugin1->initialize(node, "test_maps_manager"));

  auto plugin2 = std::make_shared<easynav::RoutesMapsManager>();
